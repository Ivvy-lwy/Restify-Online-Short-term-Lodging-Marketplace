...
const HistoricalReservations = ({change}) => {
        ...
        const fetchCommentExists = async (completedReservations) => {
            ...
            for (const reservation of completedReservations) {
                try {
                    ...
                    const response = await axios.get(`http://127.0.0.1:8000/comments/reservation/${reservation.id}/exists/`, {
                        headers: {
        ...
        const fetch_data = async () => {
            try {
                const denied = await axios.get('http://127.0.0.1:8000/reservations/list/denied/tenant/', {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                    },
                });

                const expired = await axios.get('http://127.0.0.1:8000/reservations/list/Expired/tenant/', {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                    },
                });

                const canceled = await axios.get('http://127.0.0.1:8000/reservations/list/Canceled/tenant/', {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                    },
                });

                const terminated = await axios.get('http://127.0.0.1:8000/reservations/list/Terminated/tenant/', {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                    },
                });

                const completed = await axios.get('http://127.0.0.1:8000/reservations/list/Completed/tenant/', {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                    },
                });
                            ...
                            <div className="dropdown mt-2 mt-md-0">
                                ...
                                <ul className="dropdown-menu">
                                    ...
                                    {reservation.state === "Completed" && !commentExists[reservation.id] &&
                                        <li>
                                            <Link className="dropdown-item"
                                                    to={`/comment/${reservation.property_id}/property/${reservation.id}/`}>
                                                Rate the Property
                                            </Link>
                                        </li>
                                    }
